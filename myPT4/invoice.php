...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM tbl_orders_a194333, tbl_staffs_a194333,
      tbl_customers_a194333, tbl_orders_details_a194333 WHERE
      tbl_orders_a194333.fld_staff_num = tbl_staffs_a194333.fld_staff_num AND
      tbl_orders_a194333.fld_customer_num = tbl_customers_a194333.fld_customer_num AND
      tbl_orders_a194333.fld_order_num = tbl_orders_details_a194333.fld_order_num AND
      tbl_orders_a194333.fld_order_num = :oid");
    ...
    <div class="col-xs-5">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4>From: CryptoSeven Sdn. Bhd.</h4>
            </div>
            <div class="panel-body">
                <p>
                    Aril Conglomerate Tower C, <br>
                    Avenue 1, The Orion, <br>
                    Publika, 59200<br>
                    Kuala Lumpur <br>
                </p>
            </div>
...
<table class="table table-bordered">
    ...
    try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a194333,
        tbl_products_a194333 where 
        tbl_orders_details_a194333.fld_product_num = tbl_products_a194333.fld_product_num AND
        fld_order_num = :oid");
        $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
        $oid = $_GET['oid'];
        $stmt->execute();
